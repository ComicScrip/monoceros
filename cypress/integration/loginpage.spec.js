describe("login", () => {
  ...
  it("can login with correct credentials", () => {
    cy.get("[data-cy=email]").type(Cypress.env("goodEmail"));
    cy.get("[data-cy=password]").type(Cypress.env("goodPassword"));
    cy.get("[data-cy=loginBtn]").click();
    cy.contains("This page is under construction sorry !!");
  });

  it("cannot login with wrong credentials", () => {
    cy.get("[data-cy=email]").type(Cypress.env("wrongEmail"));
    cy.get("[data-cy=password]").type(Cypress.env("wrongPassword"));
    cy.get("[data-cy=loginBtn]").click();
